...
/**
 * This root container defines a seller's return policy for a specific marketplace
 * and category type. Used when creating or updating a return policy,
 * returnPolicyRequest encapsulates a seller's terms for how buyers can return
 * items. While each seller must define at least one payment policy for every
 * marketplace into which they sell, sellers can define multiple payment policies
 * for a single marketplace by specifying different configurations for the unique
 * policies. A successful call returns a paymentPolicyId, plus the Location
 * response header contains the URI to the resource. Use the Metadata API method to
 * determine which categories in the marketplace(s) require you to provide a return
 * policy. Also note that some marketplaces require you to provide a specific
 * return policy for vehicle listings. Policy instructions can be localized by
 * providing a locale in the Content-Language HTTP request header. For example:
 * Content-Language: de-DE. Tip: For more on using business policies, see eBay
 * business policies.
 */
class ReturnPolicyRequest extends AbstractModel
{
     ...
     * feature is enabled in any given year, check the Returns on eBay page before the
     * holiday season begins. If the feature is not enabled for the season, this field
     * is ignored. The extended holiday returns period is defined by three dates: The
     * start date = start of November. The purchase cutoff date = end of the year. The
     * end date = end of January. The above dates may vary by a few days each year.
     * Sellers are notified of the current dates on their eBay marketplace before the
     * holiday period starts. Sellers can specify Extended Holiday Returns (as well as
     * their regular non-holiday returns period) for chosen listings at any time during
     * the year. The Extended Holiday Returns offer is not visible in listings until
     * the start date of current year's holiday returns period, at which point it
     * overrides the non-holiday returns policy. Buyers can see the Extended Holiday
     * Returns offer in listings displayed through the purchase cutoff date and are
     * able to return those purchases until the end date of the period. After the
     * purchase cutoff date, the Extended Holiday Returns offer automatically
     * disappears from the listings and the seller's non-holiday returns period
     * reappears. Purchases made from that point on are subject to the non-holiday
     * returns period, while purchases made before the cutoff date still have until the
     * end of the period to return under the program. If the value of holidayReturns is
     * false for an item, the return period specified by the returnsWithinOption field
     * applies, regardless of the purchase date. If the item is listed with a policy of
     * no returns, holidayReturns is automatically reset to false.
     *
     * @var bool
     */
     ...
     * only to remorse returns. Note that each eBay marketplace can support different
     * sets of refund methods. Also, each eBay marketplace has a default setting for
     * this value and if you do not specifically set this value, sellers are obligated
     * to honor the setting that displays in their listings. Call GeteBayDetails in the
     * Trading API to see what refund methods the marketplaces you sell into support.
     * We recommend you set this field to the value of your preferred refund method and
     * that you use the description field to detail the seller's return policy (such as
     * indicating how quickly the seller will process a refund, whether the seller must
     * receive the item before processing a refund, and other similar useful details).
     * You cannot modify this value in a Revise item call if (1) the listing has bids
     * or (2) the listing ends within 12 hours. For implementation help, refer to <a
     ...
     * if returnsAccepted is set to true.
     *
     ...
     * responsible for return shipping costs for SNAD-related issues. Required if
     * returnsAccepted is set to true. For implementation help, refer to <a
